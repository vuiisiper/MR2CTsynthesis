...
class UNetConvBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # print('UNetConvBlock_out1 {}'.format(out.shape))
        out = self.activation(self.bn2(self.conv2(out)))
        # print('UNetConvBlock_out2 {}'.format(out.shape))
...
class UNetUpBlock(nn.Module):
    ...
    def forward(self, x, bridge):
        up = self.up(x)
        # print('up_{}'.format(up.shape))
        up = self.activation(self.bnup(up))
        # print('up after activation_{}'.format(up.shape))
        crop1 = self.center_crop(bridge, up.size()[2])
        # print('bridge shape:{}'.format(bridge.shape))
        # print('crop1 shape:{}'.format(crop1.shape))
...
class UNet(nn.Module):
    def __init__(self, in_channel=1, n_classes=4): #what is n_classes??
        ...
        self.last = nn.Conv2d(64, n_classes, 1, stride=1) #kernel size = 1?

    def forward(self, x):
        #         ###print 'line 70 ',x.size()
        block1 = self.conv_block1_64(x)
        # print('x_{}'.format(x.shape))  #x_torch.Size([5, 1, 256, 256])
        # print('block1:{}'.format(block1.shape))
        pool1 = self.pool1(block1)
        # print('pool1:{}'.format(pool1.shape))

        block2 = self.conv_block64_128(pool1)
        # print('block2:{}'.format(block2.shape))
        pool2 = self.pool2(block2)
        # print('pool2:{}'.format(pool2.shape))


        block3 = self.conv_block128_256(pool2)
        # print('block3:{}'.format(block3.shape))
        pool3 = self.pool3(block3)
        # print('pool3:{}'.format(pool3.shape))

        block4 = self.conv_block256_512(pool3)
        # print('block4:{}'.format(block4.shape))
        pool4 = self.pool4(block4)
        # print('pool4:{}'.format(pool4.shape))

        block5 = self.conv_block512_1024(pool4)

        up1 = self.up_block1024_512(block5, block4) #self.up_block1024_512 = UNetUpBlock(1024, 512)
        # print('up1_{}'.format(up1.shape)) #5,512,32,32

        up2 = self.up_block512_256(up1, block3)
        # print('up2_{}'.format(up2.shape))

        up3 = self.up_block256_128(up2, block2)

        up4 = self.up_block128_64(up3, block1)
        # print('up4_{}'.format(up4.shape))

        # up5 = self.last(up4)
        # print('last(up4)_{}'.format(up5.shape))

        return self.last(up4) #last(up4)_torch.Size([5, 1, 256, 256])
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(1, 32, (9, 9),stride=3) #converts 1 input channel to 32 channels with 32 filters, stride = 1
        self.bn1 = nn.BatchNorm2d(32)
        self.conv2 = nn.Conv2d(32, 64, (5, 5),stride=3)
        self.bn2 = nn.BatchNorm2d(64)
        self.conv3 = nn.Conv2d(64, 64, (5, 5),stride=3)
        self.bn3 = nn.BatchNorm2d(64)
        # self.fc1 = nn.Linear(64 * 28 * 28, 64)
        # self.bn3= nn.BatchNorm1d(6)
        # self.fc2 = nn.Linear(512, 64) #or(512,128)
